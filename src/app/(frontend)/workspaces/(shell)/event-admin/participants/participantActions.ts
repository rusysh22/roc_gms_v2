...
export async function savePlayerAction(form: FormData): Promise<void> {
  ...
  const data = { event_id: Number(event.id), name, club_id: clubId ? Number(clubId) : undefined, identification_number: text(form, 'identificationNumber') || undefined, email: email || undefined, phone: text(form, 'phone') || undefined, photo: text(form, 'photo') || undefined, bio: text(form, 'bio') || undefined, gender: (gender || undefined) as 'male' | 'female' | 'other' | 'prefer_not_to_say' | undefined }
...
export async function saveTeamAction(form: FormData): Promise<void> {
  ...
  const duplicate = await payload.find({ collection: 'teams', depth: 0, limit: 10, where: { and: [{ slug: { equals: slug } }, { event_id: { equals: event.id } }] } }); if (duplicate.docs.some((team) => String(team.id) !== id)) redirect(`${page}?participantError=duplicate_slug`)
  const data = { event_id: Number(event.id), name, slug, club_id: clubId ? Number(clubId) : undefined, captain_player_id: captainId ? Number(captainId) : undefined, contact_email: email || undefined, logo: text(form, 'logo') || undefined, description: text(form, 'description') || undefined }
...
export async function saveRosterAction(form: FormData): Promise<void> {
  ...
  const data = { event_id: Number(event.id), team_id: Number(teamId), player_id: Number(playerId), category_id: categoryId ? Number(categoryId) : undefined, role: role as 'player' | 'captain' | 'coach' | 'manager' | 'substitute', status: status as 'active' | 'pending' | 'inactive' | 'withdrawn' }
