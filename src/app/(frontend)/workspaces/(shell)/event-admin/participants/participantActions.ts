...
export async function savePlayerAction(form: FormData): Promise<void> {
  ...
  const data = { event_id: Number(event.id), name, club_id: clubId ? Number(clubId) : undefined, employee_id: text(form, 'employeeId') || undefined, email: email || undefined, phone: text(form, 'phone') || undefined, photo: text(form, 'photo') || undefined, bio: text(form, 'bio') || undefined, gender: (gender || undefined) as 'male' | 'female' | 'other' | 'prefer_not_to_say' | undefined }
