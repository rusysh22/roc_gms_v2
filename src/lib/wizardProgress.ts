...
// `setup` (answered or skipped) and `history` (a passive viewer) are not tasks; `event` is
// definitionally done once an event row exists, which is a precondition for calling this at all.
...
export const deriveWizardProgress = (input: WizardProgressInput): WizardProgress => {
  const completedSteps = new Set<string>(['event', 'setup', 'history'])
